...
const FinanceMBTI: React.FC = () => {
  ...
  const renderRadarChart = (dims: Record<string, { a: number; b: number }>) => {
    const size = 280;
    const cx = size / 2;
    const cy = size / 2;
    const maxR = 100; // max radius in SVG units

    // B-side percentages: top=SD(도전), right=AQ(정량), bottom=IE(외향), left=RC(창의)
    const getBPct = (key: string) => {
      const total = dims[key].a + dims[key].b;
      return total > 0 ? (dims[key].b / total) * 100 : 50;
    };

    const axes = [
      { key: 'SD', label: 'SD\n안정-도전', angle: -Math.PI / 2 },  // top
      { key: 'AQ', label: 'AQ\n분석-정량', angle: 0 },              // right
      { key: 'IE', label: 'IE\n내향-외향', angle: Math.PI / 2 },    // bottom
      { key: 'RC', label: 'RC\n규정-창의', angle: Math.PI },         // left
    ];

    // Helper: get point on axis at given fraction (0-1)
    const getPoint = (angle: number, fraction: number) => ({
    ...
    const gridPolygons = levels.map((level) => {
      const points = axes.map((a) => getPoint(a.angle, level));
      return points.map((p) => `${p.x},${p.y}`).join(' ');
    });

    // Build data polygon
    const dataPoints = axes.map((a) => {
      const pct = getBPct(a.key) / 100;
      return getPoint(a.angle, pct);
    });
    const dataPolygon = dataPoints.map((p) => `${p.x},${p.y}`).join(' ');

    // Label positions (slightly outside the chart)
    const labelOffset = 1.22;
    const labelPositions = axes.map((a) => getPoint(a.angle, labelOffset));
        ...
        <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size} style={{ maxWidth: '100%' }}>
          ...
          {gridPolygons.map((pts, i) => (
            <polygon
              ...
              points={pts}
              fill="none"
              stroke={i === levels.length - 1 ? '#cbd5e1' : '#e2e8f0'}
          ...
          {/* Data points */}
          {dataPoints.map((p, i) => (
            <circle key={`dp-${i}`} cx={p.x} cy={p.y} r={4} fill="#7c3aed" />
          ))}
          {/* Axis labels */}
          ...
          {dataPoints.map((p, i) => {
            const pct = Math.round(getBPct(axes[i].key));
            return (
              <text
                key={`pct-${i}`}
                x={p.x}
